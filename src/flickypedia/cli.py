...
from flickypedia.backfillr import backfillr_cli
...
def run_dev_server(port: int, host: str, debug: bool) -> None:
    ...
    if app.config["OAUTH_PROVIDERS"]["wikimedia"]["client_id"] is None:
        sys.exit("No Wikimedia client ID provided! Set WIKIMEDIA_CLIENT_ID.")

    if app.config["OAUTH_PROVIDERS"]["wikimedia"]["client_secret"] is None:
